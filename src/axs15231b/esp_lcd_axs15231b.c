...
esp_lcd_new_panel_axs15231b(const esp_lcd_panel_io_handle_t io,
  ...
  case ESP_LCD_COLOR_SPACE_RGB: // Fixed enum
    axs15231b->madctl_val = 0;
    break;
  case ESP_LCD_COLOR_SPACE_BGR:               // Fixed enum
    axs15231b->madctl_val |= LCD_CMD_BGR_BIT; // Use standard bit
    break;
  default:
  ...
  *ret_panel = &(axs15231b->base);
  ESP_LOGI(TAG, "new axs15231b panel @%p", axs15231b);
  ESP_LOGI(TAG, "assigned *ret_panel @%p", *ret_panel);
...
static esp_err_t touch_axs15231b_read_data(esp_lcd_touch_handle_t tp) {
  ...
  ESP_RETURN_ON_ERROR(i2c_write_bytes(tp, -1, read_cmd, sizeof(read_cmd)), TAG,
                      "I2C write failed");
  ESP_RETURN_ON_ERROR(i2c_read_bytes(tp, -1, data, sizeof(data)), TAG,
                      "I2C read failed");
